...
If you hit a `CalledProcessError` while training, re-run with
`--embed-extra -debug -ndproc 0` to print the underlying `embed.py` stack trace
and disable multiprocessing. The passthrough flag can forward any
`embed.py` option (e.g., `-gpu -1` to force CPU or `-fresh` to discard old
checkpoints).
